import { InvalidUsernameOrPassword, PermissionDeniedError } from "../common/errors/errors";
...
class AuthorizeService {
    public async generateJWT(request: LoginRequest) {
        ...
        let token = "";
        if(decrypt(getEncryptionConfig(environment.SECRET_KEY), user.password) === request.password) {
           token = jwtUtil.sign({username: user.username, roles: user.roles}, environment.JWT_SECRET, {
            algorithm: "HS256",
        ...
        await authorizeRepository.createUserToken({username: user.username, token});
        return token;
    }
    ...
    public async verifyToken(token: string): Promise<IJWTDataResponse> {
        ...
        const jwtData = jwtUtil.verify(token, environment.JWT_SECRET) as IJWTDataResponse;
        const userToken = await authorizeRepository.getUserToken(jwtData.username, token);
        if(!userToken) {
            throw new PermissionDeniedError();
        }
        return jwtData;
