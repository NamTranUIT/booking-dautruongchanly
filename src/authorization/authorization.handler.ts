...
class AuthorizeHandler {
    ...
    public authorizer(...permittedRoles: ROLES[]) {
        return async (req: Request, res: Response, next: NextFunction) => {
            try {
                const token = req.headers.authorization.split("Bearer ")[1] || "";
                const jwtData: IJWTDataResponse = await authorizeService.verifyToken(token);
                logger.debug("User info: ", jwtData);
                const isAllowed = authorizeHandler.checkPermission(permittedRoles, jwtData.roles);
                if(!isAllowed) {
    ...
    private checkPermission(permittedRoles: ROLES[], userRoles: ROLES[]): boolean {
        let isAllowed = false;
